...
def enrich(targets, db_path, cids=None, dry_run=False, report_dir=None):
    ...
    report_lines.append("")  # separator between runs

    from datetime import date
    today = date.today().isoformat()
    report_path = os.path.join(report_dir, f"enrichment-fc2-{today}.md")
...
def main():
    ...
    fc2_target = ing.get("fc2_target")
    targets = [fc2_target] if fc2_target else []
    if not targets:
